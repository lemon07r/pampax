  ...
  if (typeof module === "object" && module !== null) {
    ...
    for (const value of values) {
      ...
      if (
        resolved &&
        resolved.language &&
        typeof resolved.language === "object"
      ) {
        return resolved;
      }
    }
...
function extractRerankerScoreHint(candidate) {
  ...
  const description =
    typeof candidate.pampa_description === "string"
      ? candidate.pampa_description
      : "";
...
export async function initDatabase(dimensions, basePath = ".") {
  ...
  await run(
    `CREATE INDEX IF NOT EXISTS idx_pampa_tags ON code_chunks(pampa_tags)`,
  );
  await run(
    `CREATE INDEX IF NOT EXISTS idx_pampa_intent ON code_chunks(pampa_intent)`,
  );
...
function extractSemanticTags(filePath, symbolName, code) {
  ...
  const pathParts = filePath
    .split("/")
    .map((part) => part.replace(/\.(php|js|ts|tsx)$/, ""));
    ...
    if (rule.variableTypes && rule.variableTypes.includes(n.type)) {
      ...
      if (isImportantVariable(varText, n.type)) {
        variables.push({
          ...
          value:
            varText.length > 100 ? varText.substring(0, 100) + "..." : varText,
        });
      }
    }
...
function extractDocComments(source, node, rule) {
  ...
  const beforeNode = source.slice(
    Math.max(0, node.startIndex - 500),
    node.startIndex,
  );
...
export async function indexProject({
  ...
  const normalizedDeleted = Array.from(
    new Set(
  ...
  const languagePatterns = getSupportedLanguageExtensions().map(
    (ext) => `**/*${ext}`,
  );
  let files = [];
  ...
  const embeddingProvider =
    embeddingProviderOverride || createEmbeddingProvider(provider);
  ...
  if (embeddingProvider.rateLimiter) {
    ...
    if (rateLimiterStats.isLimited) {
      console.log(
        `  🔒 Rate limiting: ${rateLimiterStats.rpm} requests/minute`,
      );
    } else {
      ...
      if (existingDimensions.length > 0) {
        ...
        if (hasMismatch) {
          ...
          existingDimensions.forEach((row) => {
            console.log(
              `  ${row.embedding_provider} (${row.embedding_dimensions}D)`,
            );
          });
          console.log(
            `Current config: ${currentProvider} (${currentDimensions}D)`,
          );
      ...
      codemap[chunkId] = normalizeChunkMetadata(
        {
          ...
          hasPampaTags:
            Array.isArray(pampaMetadata.tags) && pampaMetadata.tags.length > 0,
          hasIntent: !!pampaMetadata.intent,
          hasDocumentation: !!docComments,
          variableCount: Array.isArray(importantVariables)
            ? importantVariables.length
            : 0,
          encrypted: !!(writeResult && writeResult.encrypted),
          symbol_signature:
            symbolData && symbolData.signature
              ? symbolData.signature
              : undefined,
          symbol_parameters:
            symbolData && Array.isArray(symbolData.parameters)
              ? symbolData.parameters
              : undefined,
          symbol_return:
            symbolData && symbolData.returnType
              ? symbolData.returnType
              : undefined,
          symbol_calls:
            symbolData && Array.isArray(symbolData.calls)
              ? symbolData.calls
              : undefined,
        },
        previousMetadata,
  ...
  async function removeFileArtifacts(fileRel) {
    const entries = Object.entries(codemap).filter(
      ([, metadata]) => metadata && metadata.file === fileRel,
    );
  ...
  for (const rel of files) {
    ...
    const existingChunks = new Map(
      Object.entries(codemap).filter(
        ([, metadata]) => metadata && metadata.file === rel,
      ),
    );
    ...
    try {
        ...
        if (source.length > SIZE_THRESHOLD) {
          tree = parser.parse((index, position) => {
            if (index < source.length) {
              return source.slice(
                index,
                Math.min(index + CHUNK_SIZE, source.length),
              );
            }
            return null;
          });
      ...
      async function processNodeGroup(nodeGroup) {
        ...
        if (combinedChunk) {
          ...
          chunkingStats.functionsGrouped =
            (chunkingStats.functionsGrouped || 0) + nodeGroup.nodes.length;
      ...
      async function yieldChunk(node, parentNode = null) {
        ...
        if (
          ...
          analysis.subdivisionCandidates.length > 0
        ) {
          ...
          if (smallChunks.length > 0) {
            const totalSmallSize = smallChunks.reduce(
              (sum, c) => sum + c.size,
              0,
            );
      ...
      async function processChunk(
        ...
        const importantVariables = extractImportantVariables(
          node,
          source,
          rule,
        );
        ...
        const chunkType = node.type.includes("class")
          ? "class"
  ...
  if (chunkingStats.totalNodes > 0) {
    ...
    if (chunkingStats.fileGrouped && chunkingStats.functionsGrouped) {
      console.log(
        `  🎯 File-grouped chunks: ${chunkingStats.fileGrouped} (${chunkingStats.functionsGrouped} functions combined)`,
      );
    }

    console.log(
      `  Normal chunks (optimal size): ${chunkingStats.normalChunks || 0}`,
    );
    console.log(`  Subdivided (too large): ${chunkingStats.subdivided || 0}`);
    console.log(`  Merged small chunks: ${chunkingStats.mergedSmall || 0}`);
    console.log(
      `  Statement-level fallback: ${chunkingStats.statementFallback || 0}`,
    );
    console.log(`  Skipped (too small): ${chunkingStats.skippedSmall || 0}`);
    console.log(`  Final chunk count: ${processedChunks}`);

    const reductionRatio =
      chunkingStats.totalNodes > 0
        ? ((1 - processedChunks / chunkingStats.totalNodes) * 100).toFixed(1)
        : 0;
    console.log(
      `  Chunk reduction: ${reductionRatio}% fewer chunks vs naive approach`,
    );
    console.log("");
  }
  ...
  if (modelProfile.useTokens && tokenStats.totalRequests > 0) {
    ...
    console.log(
      `  Character pre-filter: ${tokenStats.charFilterRate} (${tokenStats.charFilterSkips} skipped)`,
    );
    console.log(
      `  Cache hits: ${tokenStats.cacheHitRate} (${tokenStats.cacheHits} cached)`,
    );
    ...
    const efficiency =
      tokenStats.totalRequests > 0
        ? (
            ((tokenStats.charFilterSkips + tokenStats.cacheHits) /
              tokenStats.totalRequests) *
            100
          ).toFixed(1)
        : 0;
    console.log(
      `  Overall efficiency: ${efficiency}% avoided expensive tokenization`,
    );
    console.log("");
  }
...
export async function searchCode(
  ...
  try {
    ...
    for (const chunk of scopedChunks) {
      ...
      if (
        chunk.pampa_intent &&
        query.toLowerCase().includes(chunk.pampa_intent.toLowerCase())
      ) {
        boostScore += 0.2;
      }
    ...
    const filteredResults = sortedResults.filter(
      (result) => !intentionChunkIds.has(result.id),
    );
    ...
    if (remainingSlots > 0) {
        ...
        if (bm25Index) {
          ...
          if (bm25Results.length > 0) {
            const fused = reciprocalRankFusion({
              vectorResults: vectorPool.map((item) => ({
                id: item.id,
                score: item.score,
              })),
              bm25Results: bm25Results.map((item) => ({
                id: item.id,
                score: item.score,
              })),
      ...
      const hasSymbolBoost =
        ...
        vectorResults.some(
          (candidate) =>
            typeof candidate.symbolBoost === "number" &&
            candidate.symbolBoost > 0,
        );
      ...
      if (
        ...
        (normalizedScope.reranker === "transformers" ||
          normalizedScope.reranker === "api")
      ) {
        try {
          ...
          if (
            ...
            reranked.length === vectorResults.length
          ) {
    ...
    const combinedResults = [
      ...
      ...vectorResults.map((result) => {
        ...
        if (typeof result.symbolBoost === "number" && result.symbolBoost > 0) {
          ...
          if (
            Array.isArray(result.symbolBoostSources) &&
            result.symbolBoostSources.length > 0
          ) {
    ...
    if (combinedResults.length === 0) {
      return {
        ...
        suggestion:
          "Try broader search terms or check if the project is properly indexed",
        provider: embeddingProvider.getName(),
        scope: normalizedScope,
        hybrid: { enabled: hybridEnabled, bm25Enabled },
        symbolBoost: { enabled: symbolBoostEnabled, boosted: false },
    ...
    if (
      ...
      combinedResults[0].meta.score > 0.8
    ) {
      // Record the top result as a successful intention mapping
      await recordIntention(
        query,
        combinedResults[0].sha,
        combinedResults[0].meta.score,
        workingPath,
      );
    }
    ...
    return {
      ...
      query,
      searchType:
        bm25Fused || scopedIntentionResults.length > 0 ? "hybrid" : "vector",
...
export async function getChunk(sha, workingPath = ".") {
  ...
  try {
    ...
    if (!result) {
      ...
      throw new Error(
        `Chunk ${sha} not found at ${plainPath} or ${encryptedPath}`,
      );
    }
...
export async function searchByIntention(query, workingPath = ".") {
  ...
  try {
    ...
    if (
      ...
      error.message.includes("no such table: intention_cache")
    ) {
      return {
        success: false,
        directMatch: false,
        error: "intention_cache_missing",
      };
    }
