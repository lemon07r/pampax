...
class ErrorLogger {
  ...
  async logAsync(error, context = {}) {
    const timestamp = new Date().toISOString();
    const logEntry =
      `[${timestamp}] ASYNC ERROR: ${error.message}\n` +
      `Working Path: ${path.resolve(this.workingPath)}\n` +
      `Context: ${JSON.stringify(context, null, 2)}\n` +
      `Stack: ${error.stack}\n` +
      `${"=".repeat(80)}\n\n`;
    ...
    try {
      ...
      console.error(
        `❌ Async error logged to ${this.errorLogPath}:`,
        error.message,
      );

      if (debugMode) {
        this.debugLog("Async error occurred", {
          error: error.message,
          context,
        });
      }
...
function validateEnvironment(workingPath = ".") {
  ...
  if (!fs.existsSync(pampaDir)) {
    errors.push(
      `.pampa directory not found in ${workingPath}. Run index_project first.`,
    );
  }
  ...
  if (!fs.existsSync(dbPath)) {
    errors.push(
      `Database .pampa/pampa.db not found in ${workingPath}. Run index_project first.`,
    );
  }
...
server.tool(
  "search_code",
  {
    query: z
      .string()
      .min(2, "Query cannot be empty")
      .describe(
        "Semantic search query (e.g. 'authentication function', 'error handling')",
      ),
    limit: z
      .number()
      .optional()
    ...
    provider: z
      ...
      .optional()
      .default("auto")
      .describe("Embedding provider (auto|openai|transformers|ollama|cohere)"),
    path: z
      .string()
      .optional()
      .default(".")
      .describe(
        "PROJECT ROOT directory path where PAMPAX database is located (the directory containing .pampa/ folder)",
      ),
    path_glob: z
      .union([z.string(), z.array(z.string())])
      .optional()
      .describe("Optional glob pattern(s) to limit search scope"),
    tags: z
    ...
    lang: z
      .union([z.array(z.string()), z.string()])
      .optional()
      .describe("Optional list of languages to filter results"),
    reranker: z
      .enum(["off", "transformers", "api"])
      .optional()
      .describe("Optional reranker strategy flag (off|transformers|api)"),
    hybrid: z
      .enum(["on", "off"])
      .optional()
      .default("on")
      .describe("Enable hybrid semantic + keyword fusion"),
    bm25: z
      .enum(["on", "off"])
      .optional()
      .default("on")
      .describe("Enable BM25 keyword retrieval stage"),
    symbol_boost: z
      .enum(["on", "off"])
      .optional()
      .default("on")
      .describe("Enable symbol-aware ranking boost"),
  },
  async ({
    query,
    limit,
    ...
    try {
      ...
      if (!query || typeof query !== "string") {
        await errorLogger.logAsync(
          new Error("Query undefined or invalid type"),
          {
            ...context,
            receivedQuery: query,
        ...
        content: [
          {
            type: "text",
            text:
              `Found ${results.results.length} results for: "${cleanQuery}"\n` +
              packLine +
              `Provider: ${results.provider}\n` +
...
server.tool(
  "get_code_chunk",
  {
    sha: z
      .string()
      .min(1, "SHA cannot be empty")
      .describe(
        "SHA of the code chunk to retrieve (obtained from search_code results)",
      ),
    path: z
      .string()
      .optional()
      .default(".")
      .describe(
        "PROJECT ROOT directory path where PAMPAX chunks are stored (same path used in search_code)",
      ),
  },
          ...
          content: [
            {
              type: "text",
              text:
                `ERROR: SHA is required and must be a valid string.\n\n` +
          ...
          content: [
            {
              type: "text",
              text:
                `⚠️ CODE CHUNK TOO LARGE - TRUNCATED\n\n` +
                `SHA: ${cleanSha}\n` +
        ...
        content: [
          {
            type: "text",
            text:
              `ERROR getting chunk: ${error.message}\n\n` +
              `Details:\n` +
              `- Requested SHA: ${sha}\n` +
...
server.tool(
  "index_project",
  {
    path: z
      ...
      .default(".")
      .describe(
        "PROJECT ROOT directory path to index (will create .pampa/ subdirectory here)",
      ),
    provider: z
      .string()
      .optional()
          ...
          content: [
            {
              type: "text",
              text:
                `Indexing failed: ${result.message || "Unknown error"}\n\n` +
...
server.tool(
  "get_project_stats",
  {
    path: z
      ...
      .default(".")
      .describe(
        "PROJECT ROOT directory path where PAMPAX database is located (same path used in index_project)",
      ),
  },
          ...
          content: [
            {
              type: "text",
              text:
                `Error getting overview: ${overviewResult.message}\n\n` +
...
server.tool(
  "update_project",
  {
    path: z
      ...
      .default(".")
      .describe(
        "PROJECT ROOT directory path to update (same directory where you ran index_project)",
      ),
    provider: z
      .string()
      .optional()
  ...
  async ({ path: projectPath, provider }) => {
    ...
    try {
      ...
      if (result.processedChunks === 0) {
        ...
        responseText =
          `🔄 Project ${operationText} successfully!\n\n` +
          `📊 Statistics:\n` +
          `- Processed chunks: ${result.processedChunks}\n` +
          `- Total chunks: ${result.totalChunks}\n` +
          `- Provider: ${result.provider}\n\n` +
 ...
 */
server.resource("codemap", "pampa://codemap", async (uri) => {
  try {
 ...
 */
server.resource("overview", "pampa://overview", async (uri) => {
  try {
    ...
    const overview = results.results
      .map(
        (result) => `${result.path} :: ${result.meta.symbol} (${result.lang})`,
      )
      .join("\n");
...
server.prompt(
  "analyze_code",
  {
    ...
    focus: z
      ...
      .optional()
      .describe(
        "Specific aspect to analyze (e.g. 'security', 'performance', 'bugs')",
      ),
  },
  ({ query, focus }) => ({
    messages: [
      {
        ...
        content: {
          type: "text",
          text:
            `Analyze code related to: "${query}"${focus ? ` with focus on: ${focus}` : ""}\n\n` +
            `Steps to follow:\n` +
...
server.prompt(
  "find_similar_functions",
  {
    functionality: z
      .string()
      .describe("Description of the functionality sought"),
  },
  ({ functionality }) => ({
    messages: [
      {
        ...
        content: {
          type: "text",
          text:
            `Find existing functions that implement: "${functionality}"\n\n` +
            `Steps:\n` +
...
process.on("unhandledRejection", (reason, promise) => {
  ...
  errorLogger.log(new Error(reason), {
