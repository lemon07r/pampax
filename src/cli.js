...
program
  .name("pampa")
  .description(
    "PAMPA - Protocol for Augmented Memory of Project Artifacts (MCP)",
  )
...
program
  ...
  .option(
    "-p, --provider <provider>",
    "embedding provider (auto|openai|transformers|ollama|cohere)",
    "auto",
  )
  .option(
    "--project <path>",
    "alias for project path (same as [path] argument)",
  )
  .option(
    "--directory <path>",
    "alias for project directory (same as [path] argument)",
  )
  .option(
    "--encrypt <mode>",
    "encrypt chunk payloads when PAMPAX_ENCRYPTION_KEY is configured (on|off)",
  )
  .action(async (projectPath = ".", options) => {
    const resolvedPath =
      options.project || options.directory || projectPath || ".";
...
program
  .command("update [path]")
  .description(
    "Update index by re-scanning all files (recommended after code changes)",
  )
  .option(
    "-p, --provider <provider>",
    "embedding provider (auto|openai|transformers|ollama|cohere)",
  ...
  .action(async (projectPath = ".", options) => {
    ...
    try {
      ...
      console.log(
        "💡 Your AI agents now have access to the latest code changes",
      );
    } catch (error) {
...
program
  ...
  .description(
    "Search indexed code with optional path/tag/lang filters, provider overrides, reranker, and symbol-aware boosts",
  )
  ...
  .option(
    "--path_glob <pattern...>",
    "limit results to files matching the provided glob pattern(s)",
  )
  .option(
    "--tags <tag...>",
    "filter results to chunks tagged with the provided values",
  )
  .option(
    "--lang <language...>",
    "filter results to the specified languages (e.g. php, ts)",
  )
  .option(
    "--reranker <mode>",
  ...
  .option(
    "--hybrid <mode>",
    "toggle reciprocal-rank-fused hybrid search (on|off)",
    "on",
  )
  .option(
    "--bm25 <mode>",
    "toggle BM25 keyword candidate generation (on|off)",
    "on",
  )
  .option(
    "--symbol_boost <mode>",
    "toggle symbol-aware ranking boost (on|off)",
    "on",
  )
  .addHelpText(
    "after",
  ...
  .action(async (query, projectPath = ".", options) => {
    try {
      ...
      const { scope: scopeFilters, pack } = buildScopeFiltersFromOptions(
        options,
        resolvedPath,
      );
      if (pack) {
      ...
      console.log(
        'TIP: Use "pampa mcp" to start the MCP server and get the complete code',
      );
    } catch (error) {
...
program
  ...
  .action(async () => {
    try {
      ...
      console.log(`🏷️  PAMPAX version: ${version}`);
